...
export default class CanadaCovidInfo {
    ...
    request(`https://api.opencovid.ca/summary?loc=${province}`, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        for (let elem in data) {
          let active_cases_per_million =
            (1000000 *
              (data[elem]['cumulative_cases'] -
                data[elem]['cumulative_deaths'] -
                data[elem]['cumulative_recovered'])) /
            population[province];
          let active_cases_per_million_safe = active_cases_per_million <= baselineActiveCases;

          let fully_vaccinated_proportion =
            data[elem]['cumulative_cvaccine'] / population[province];
          let fully_vaccinated_proportion_safe =
            fully_vaccinated_proportion >= baselineFullyVacxxed;

          let vulnerable_proportion =
            (population[province] - data[elem]['cumulative_cvaccine']) / population[province] +
            vaccinatedRiskConstant;
          let vulnerable_proportion_safe = vulnerable_proportion <= baselineVulnerable;

          let isSafe = false;

          if (
            (active_cases_per_million_safe && fully_vaccinated_proportion_safe) ||
          ...
          return new request('https://api.opencovid.ca/other?stat=hr', function (
            regionError,
            regionResponse,
            regionBody
          ) {
  ...
  static async asyncGetCovidInfoRegion(req, res, next) {
    ...
    request(
      'https://api.covid19tracker.ca/summary/split/hr',
      function (hrError, regionsJson, hrBody) {
                ...
                if (!popError && popResponse.statusCode == 200) {
                  ...
                  for (var i in regionList) {
                    if (regionList[i]['HR_UID'] == regionId) {
                      ...
                      return new request(
                        `https://api.opencovid.ca/timeseries?stat=cases&loc=${regionId}&date=${dateStr}`,
                        function (casesError, casesResponse, casesBody) {
                          if (!casesError && casesResponse.statusCode == 200) {
                            let caseData = JSON.parse(casesBody)['cases'][0];

                            if (caseData.length !== 0) {
                              let active_cases_per_million =
                                (1000000 * caseData['cases']) / regionPop;
                              let active_cases_per_million_safe =
                                active_cases_per_million <= baselineActiveCases;

                              let fully_vaccinated_proportion =
                                data[elem]['total_vaccinated'] / regionPop;
                              let fully_vaccinated_proportion_safe =
                                fully_vaccinated_proportion >= baselineFullyVacxxed;

                              let vulnerable_proportion =
                                (regionPop - data[elem]['total_vaccinated']) / regionPop +
                                vaccinatedRiskConstant;
                              let vulnerable_proportion_safe =
                                vulnerable_proportion <= baselineVulnerable;

                              let isSafe = false;

                              if (
                                (active_cases_per_million_safe &&
                                  fully_vaccinated_proportion_safe) ||
                                (active_cases_per_million_safe && vulnerable_proportion_safe) ||
                                (fully_vaccinated_proportion_safe && vulnerable_proportion_safe)
                              ) {
                                isSafe = true;
                              ...
                              finalResponse.active_cases_per_million_safe =
                                active_cases_per_million_safe;
                              finalResponse.fully_vaccinated_proportion =
                                fully_vaccinated_proportion;
                              finalResponse.fully_vaccinated_proportion_safe =
                                fully_vaccinated_proportion_safe;
  ...
  static async asyncGetCovidInfoProvinceAllRegion(req, res, next) {
    ...
    const regionsResponse = await fetch(
      `https://api.covid19tracker.ca/province/${province}/regions`
    );

    const regionList = await regionsResponse.json().catch((e) => console.log(e));
    ...
    for (var elem in regionList) {
      fetchCalls.push(
        fetch(
          `http://localhost:5000/api/v1/CanadaCovidInfo/${province}/${regionList[elem]['hr_uid']}/regionInfo`
